...
export function CreateOpportunityModal() {
  ...
  return (
    <div className="modal-backdrop" onClick={e => e.target === e.currentTarget && closeCreate()}>
      <div className="modal fade-up">
        <div className="modal-header">
          ...
          <button className="modal-close" onClick={closeCreate}><X size={16} /></button>
        </div>
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
          <div className="modal-body">
            <div className="field">
              ...
              <input {...register('titulo')} className={`field-input ${errors.titulo ? 'error' : ''}`} placeholder="e.g. Website Redesign Project" />
              {errors.titulo && <span className="field-error">{errors.titulo.message}</span>}
            </div>

            <div className="field">
              <label className="field-label">Client *</label>
              <input {...register('cliente')} className={`field-input ${errors.cliente ? 'error' : ''}`} placeholder="e.g. Acme Corporation" />
              {errors.cliente && <span className="field-error">{errors.cliente.message}</span>}
            </div>

            <div className="field-row">
              <div className="field">
                <label className="field-label">Value (USD) *</label>
                <input {...register('valor', { valueAsNumber: true })} type="number" min="0" step="0.01" className={`field-input ${errors.valor ? 'error' : ''}`} placeholder="50000" />
            ...
            <div className="field">
              ...
              <textarea {...register('descricao')} className="field-textarea" placeholder="Brief description..." />
          ...
          <div className="modal-footer">
            ...
            <button type="submit" className="btn-submit" disabled={isPending}>
              {isPending ? <><Loader2 size={15} className="spin" /> Creating...</> : 'Create Opportunity'}
            </button>
          </div>
        </form>
      </div>
